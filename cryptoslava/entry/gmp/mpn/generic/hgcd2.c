...
static inline mp_limb_t div_simple(mp_limb_t *rp, mp_limb_t a, mp_limb_t b)
{
	/*mp_limb_signed_t q = (mp_limb_signed_t) a / (mp_limb_signed_t) b;*/
	/*mp_limb_signed_t r = (mp_limb_signed_t) a % (mp_limb_signed_t) b;*/
	mp_limb_t q = a / b;
	mp_limb_t r = a % b;
	/**rp = r >= 0 ? r : (b < 0 ? -b : b) + r;*/
	*rp = r;
...
static void mul21_simple(mp_ptr prod, mp_limb_t ah, mp_limb_t al, mp_limb_t b)
{
	mp_limb_t ph, pl, p2h, p2l;
	/*ph = ah * b;*/
	/*umul_ppmm(ph, pl, ah, b);*/
	umul_ppmm(ph, pl, al, b);
	ph += ah * b;


}

static int cmp2(mp_limb_t ah, mp_limb_t al, mp_limb_t bh, mp_limb_t bl)
{
	if (ah > bh)
		return 1;
	if (ah < bh)
...
static mp_limb_t div2_simple(mp_ptr rp, mp_limb_t nh, mp_limb_t nl,
      mp_limb_t dh, mp_limb_t dl)
{
	/*__uint128_t n = ((__uint128_t) nh << 64) | nl;*/
	/*__uint128_t d = ((__uint128_t) dh << 64) | dl;*/
	/*__uint128_t q = n / d, r = n - q*d;*/
	/*rp[0] = r;*/
	/*rp[1] = r >> 64;*/

	/*mp_limb_t q, n[2] = {nl, nh}, d[2] = {dl, dh};*/
	/*mpn_tdiv_qr(&q, rp, 0, n, 2, d, 2);*/
	int count;
	mp_limb_t d2, n2h, n2l, q, bad_r, n3h, n3l, full_q;

	/* Quick estimate of q */
	full_q = nh / (dh + 1);
	/* n3 = d * full_q */
	umul_ppmm(n3h, n3l, dl, full_q);
	n3h += dh * full_q;
	sub_ddmmss(nh, nl, nh, nl, n3h, n3l);
	if (LIKELY(cmp2(nh, nl, dh, dl) < 0))
		goto out;

	/* Restore n */
	/*add_ssaaaa(nh, nl, nh, nl, n3h, n3l);*/
	/*full_q = 0;*/
	/* Loop until remaninder < d */
	do {
		/* Construct d2, n2{h,l} s. t. high bit of d2 is set.
		 * q = n2 / d2 */
		count_leading_zeros(count, dh);
		d2 = ((dh << count) | (dl >> (sizeof(dh)*8 - count))) + 1;
		n2h = nh >> (sizeof(nh)*8 - count);
		n2l = (nh << count) | (nl >> (sizeof(nl)*8 - count));
		udiv_qrnnd(q, bad_r, n2h, n2l, d2);
		/* TODO: handle theoretical q == 0 */

/*try_quick:*/
		/* n3 = d * q */
		umul_ppmm(n3h, n3l, dl, q);
		n3h += dh * q;
		full_q += q;

		/* n -= n3 */
		sub_ddmmss(nh, nl, nh, nl, n3h, n3l);
	} while (UNLIKELY(cmp2(nh, nl, dh, dl) >= 0));
	   ...
	   struct hgcd_matrix1 *M)
{
  ...
  for (;;)
      ...
      else
	{
	  ...
	  mp_limb_t q = div2_simple (r, ah, al, bh, bl);
	  al = r[0]; ah = r[1];
	  if (ah < 2)
	    {
      ...
      else
	{
	  ...
	  mp_limb_t q = div2_simple (r, bh, bl, ah, al);
	  bl = r[0]; bh = r[1];
	  if (bh < 2)
	    {
  ...
  for (;;)
      ...
      else
	{
	  ...
	  mp_limb_t q = div_simple (&r, ah, bh);
	  ah = r;
	  if (ah < (CNST_LIMB(1) << (GMP_LIMB_BITS / 2 + 1)))
	    {
	      /* A is too small, but q is correct. */
      ...
      else
	{
	  ...
	  mp_limb_t q = div_simple (&r, bh, ah);
	  bh = r;
	  if (bh < (CNST_LIMB(1) << (GMP_LIMB_BITS / 2 + 1)))
	    {
	      /* B is too small, but q is correct. */
